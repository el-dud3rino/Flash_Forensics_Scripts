...
" style="height:32px; width:32px; border-radius:50%; object-fit:cover; border: 2px solid var(--accent);">
    ...
    <script>
        ...
        function aiBuildPanel(container){
            ...
            +     '</div>'
            +     '<div></div>'
            +     '<select id="aiModelSelect" onchange="aiPickModel()" hidden style="width:100%; padding:6px; background:var(--bg-color); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px;"></select>'
            +     '<div></div>'
