    ...
    <script>
        ...
        function openPlaybook(name) {
            ...
            newWin.document.write(`
                ...
                <style>html { background: #0f172a; color-scheme: dark; } body { font-family: -apple-system, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 32px 24px; line-height: 1.65; max-width: 900px; margin: 0 auto; color: #cbd5e1; background: #0f172a; } a { color: #60a5fa; text-decoration: none; } a:hover { text-decoration: underline; } h1, h2, h3, h4, h5, h6 { color: #f8fafc; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 6px; margin: 28px 0 12px; scroll-margin-top: 16px; } h1 { color: #60a5fa; } h4, h5, h6 { border-bottom: none; margin-top: 18px; } pre { background: #1e293b; border: 1px solid rgba(255,255,255,0.1); padding: 12px; overflow-x: auto; border-radius: 6px; } code { background: #1e293b; padding: 2px 5px; border-radius: 3px; font-family: Consolas, 'Courier New', monospace; color: #e2e8f0; } pre code { background: none; padding: 0; } table { border-collapse: collapse; width: 100%; margin-bottom: 20px; } th, td { border: 1px solid rgba(255,255,255,0.15); padding: 8px; text-align: left; } th { background: #1e293b; color: #f8fafc; } strong { color: #f1f5f9; } hr { border: none; border-top: 1px solid rgba(255,255,255,0.1); margin: 24px 0; } ul, ol { padding-left: 22px; } li { margin: 3px 0; }</style>
                </head><body>
                <div id="content"></div>
                <script>
                    if (typeof marked !== 'undefined') {
                        var contentEl = document.getElementById('content');
                        contentEl.innerHTML = marked.parse(${JSON.stringify(md)});
                        // marked does not emit heading ids, so the Table of Contents anchor
                        // links have nothing to jump to. Assign GitHub-style slugs (lowercase,
                        // punctuation stripped, spaces to hyphens, -N suffix for duplicates).
                        (function(){
                            var seen = {};
                            function slugify(t){ return t.trim().toLowerCase().replace(/[^\\w\\s-]/g,'').replace(/\\s/g,'-'); }
                            var heads = contentEl.querySelectorAll('h1,h2,h3,h4,h5,h6');
                            for (var i=0; i<heads.length; i++){
                                var s = slugify(heads[i].textContent);
                                if (seen[s] === undefined){ seen[s] = 0; } else { seen[s]++; s = s + '-' + seen[s]; }
                                heads[i].id = s;
                            }
                        })();
                    } else {
        ...
        function aiPickModel(){
            const sel = document.getElementById('aiModelSelect');
            if (sel && sel.value){ document.getElementById('aiModel').value = sel.value; aiSet('ff_ai_model', sel.value); }
        }
        ...
        async function aiLoadModels(){
            ...
            try {
                ...
                const sel = document.getElementById('aiModelSelect');
                if (sel){
                    const cur = document.getElementById('aiModel').value.trim();
                    sel.innerHTML = '<option value="">-- pick a model (' + ids.length + ') --</option>'
                        + ids.map(function(id){ return '<option value="' + aiEsc(id) + '"' + (id === cur ? ' selected' : '') + '>' + aiEsc(id) + '</option>'; }).join('');
                    sel.hidden = false;
                }
                statusEl.textContent = ids.length + ' models available - pick one from the dropdown below.';
            } catch(e){
        ...
        // ---- Copy-paste PowerShell (Invoke-RestMethod) fallback ----
        // For locked networks where the browser can't call the gateway (no CORS) and
        // the local proxy can't run (Constrained Language Mode blocks HttpListener).
        // Invoke-RestMethod is a cmdlet, so it works in CLM, and PowerShell is not a
        // browser, so CORS does not apply.
        function aiPsEsc(s){ return String(s == null ? '' : s).replace(/'/g, "''"); } // PowerShell single-quote escaping
        function aiBuildPsCommand(){
            const provider = document.getElementById('aiProvider').value;
            const preset = AI_PRESETS[provider] || AI_PRESETS.custom;
            const url = document.getElementById('aiBaseUrl').value.trim();
            ...
            const q = document.getElementById('aiQuestion').value.trim();
            const scope = document.getElementById('aiScope').value;
            if (!url || !model){ return { err: 'Set a Base URL and Model in AI Settings first.' }; }
            if (!q){ return { err: 'Type a question.' }; }
            const ctx = aiBuildContext(scope);
            if (!ctx.data){ return { err: ctx.label + '. Select a host from the sidebar, or choose "Everything loaded".' }; }
            const userText = 'QUESTION:\n' + q + '\n\nDATA (' + ctx.label + '):\n```json\n' + JSON.stringify(ctx.data) + '\n```';
            let body, headers, resultExpr;
            if (preset.format === 'anthropic'){
                body = JSON.stringify({ model: model, max_tokens: 4096, system: AI_SYSTEM_PROMPT, messages: [{ role: 'user', content: userText }] });
                headers = "@{ 'x-api-key' = '" + aiPsEsc(key || 'YOUR_API_KEY') + "'; 'anthropic-version' = '2023-06-01' }";
                resultExpr = '$r.content[0].text';
            } else {
                body = JSON.stringify({ model: model, messages: [{ role: 'system', content: AI_SYSTEM_PROMPT }, { role: 'user', content: userText }] });
                headers = "@{ Authorization = 'Bearer " + aiPsEsc(key || 'YOUR_API_KEY') + "' }";
                resultExpr = '$r.choices[0].message.content';
            }
            // Single-quoted here-string keeps the JSON literal (compact JSON is one line,
            // so nothing inside can start a line with the '@ terminator).
            const cmd =
                "$body = @'\n" + body + "\n'@\n" +
                "$r = Invoke-RestMethod -Uri '" + aiPsEsc(url) + "' -Method Post -ContentType 'application/json' -Headers " + headers + " -Body $body\n" +
                resultExpr;
            return { cmd: cmd, chars: cmd.length };
        }
        function aiCopyPowerShell(){
            const errEl = document.getElementById('aiError'); errEl.textContent = '';
            const out = aiBuildPsCommand();
            if (out.err){ errEl.textContent = out.err; return; }
            aiSaveSettings();
            const ta = document.getElementById('aiPsText');
            ta.value = out.cmd;
            const note = document.getElementById('aiPsNote');
            if (note) note.textContent = 'Paste into PowerShell and run (works in Constrained Language Mode; no browser/CORS). ~' + out.chars.toLocaleString() + ' chars; includes your API key.';
            document.getElementById('aiPsBox').style.display = 'block';
            aiCopyPsAgain();
        }
        function aiCopyPsAgain(){
            const ta = document.getElementById('aiPsText');
            if (!ta) return;
            ta.focus(); ta.select();
            let ok = false;
            try { ok = document.execCommand('copy'); } catch(e){}
            if (!ok && navigator.clipboard){ try { navigator.clipboard.writeText(ta.value); } catch(e){} }
        }
        ...
        function aiCopyConversation(btn){
            if (!window.aiConversation || window.aiConversation.length === 0){ return; }
            const lines = [];
            window.aiConversation.forEach(function(e){
                lines.push('You (' + (e.scope || '') + '):');
                lines.push(e.q || '');
                lines.push('');
                if (e.pending){ lines.push('Assistant: (pending...)'); }
                else if (e.error){ lines.push('Error: ' + e.error); }
                else { lines.push('Assistant:'); lines.push(e.a || ''); }
                lines.push('');
                lines.push('----------------------------------------');
                lines.push('');
            });
            const ok = aiCopyText(lines.join('\n'));
            if (btn){
        ...
        function aiBuildPanel(container){
            ...
            +       '<input id="aiModel" value="'+aiEsc(model)+'" placeholder="model id" style="flex:1; padding:6px; background:var(--bg-color); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px;">'
            ...
            +     '</div>'
            +     '<select id="aiModelSelect" onchange="aiPickModel()" hidden style="grid-column:1 / -1; width:100%; padding:6px; background:var(--bg-color); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px;"></select>'
            +     '<div></div>'
            ...
            +   '</select>'
            +   '<button onclick="aiCopyConversation(this)" style="margin-left:auto; padding:6px 12px; background:var(--bg-lighter); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px; cursor:pointer;">Copy conversation</button>'
            +   '<button onclick="aiClearConversation()" style="margin-left:8px; padding:6px 12px; background:var(--bg-lighter); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px; cursor:pointer;">Clear conversation</button>'
            + '</div>'
            ...
            + '<div style="margin-top:8px;"><button onclick="aiCopyPowerShell()" title="Build an Invoke-RestMethod command to paste into PowerShell (for Constrained Language Mode / locked networks where the browser and proxy are blocked)" style="padding:6px 12px; background:var(--bg-lighter); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px; cursor:pointer;">&#128203; Copy as PowerShell command</button></div>'
            + '<div id="aiPsBox" style="display:none; margin-top:8px;">'
            +   '<div id="aiPsNote" style="font-size:0.75rem; color:var(--text-muted); margin-bottom:4px;"></div>'
            +   '<textarea id="aiPsText" readonly rows="6" onclick="this.select()" style="width:100%; box-sizing:border-box; font-family:monospace; font-size:0.72rem; padding:8px; background:var(--bg-color); color:var(--text-main); border:1px solid var(--glass-border); border-radius:4px;"></textarea>'
            +   '<button onclick="aiCopyPsAgain()" style="margin-top:6px; padding:5px 12px; background:var(--accent); color:white; border:none; border-radius:4px; cursor:pointer;">Copy again</button>'
            + '</div>'
            + '</div>';
