...
- **Ask AI (Optional)**: An opt-in **🤖 Ask AI** tab lets an analyst ask natural-language questions about the collected data. It is provider-agnostic — pick a preset (Anthropic Claude, OpenAI, or a **GenAI.mil**/custom OpenAI-compatible gateway), paste your own endpoint/model/key, and choose how much context to send (current host + tab, the whole host, or every loaded host). This is the **only** feature that makes a network call; the rest of the dashboard stays fully offline. Your API key is stored only in that browser's `localStorage` — it is never written to the dashboard files or committed. Because a question ships forensic data to the chosen endpoint, only point it at an endpoint approved for that data.
