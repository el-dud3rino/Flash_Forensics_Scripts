...
1.  **Configuration (client-side only)**: The analyst selects a provider preset (Anthropic Claude, OpenAI, Google Gemini, or a GenAI.mil / custom OpenAI-compatible gateway) and supplies a Base URL, model id, and API key. Gemini uses its OpenAI-compatible endpoint (`/v1beta/openai/chat/completions`), so it flows through the same OpenAI-format transport rather than a bespoke branch. A **Load models** action derives the endpoint's `GET /v1/models` URL, queries it, and populates the model field's autocomplete with the exact available model ids — useful for a gateway like GenAI.mil that fronts multiple model families (OpenAI, Gemini, Grok). All four values persist **only** in the browser's `localStorage` (`ff_ai_*` keys); they are never written into `index.html`/`data.js` and never committed. No key or endpoint is embedded in the generated dashboard.
2.  **Context scoping**: Before each question the analyst chooses how much data to attach, across five levels: current host + current tab (a minimal slice of the selected dataset); current host + current tab across **all timelines** (every temporal dataset for that host, reduced to just the current tab's categories and each tagged with its `Timestamp` — ideal for "what changed on this tab over time"); the entire current host (one dataset, all categories); the entire current host across **all timelines**; or every loaded host. The all-timeline scopes read `groupedSystems[selectedHostname]` (the full set of temporal captures) rather than the single selected `currentComputer`. The engine serializes only the chosen slice to JSON and displays an estimated size (chars / tokens) with a warning past ~500K characters, so nothing is sent silently.
3.  **Request transport**: A raw `fetch` is issued from the browser. For the Anthropic format it POSTs to `/v1/messages` with `x-api-key`, `anthropic-version: 2023-06-01`, and `anthropic-dangerous-direct-browser-access: true`, and reads the `content[].text` blocks. For the OpenAI-compatible format it POSTs to the gateway's `/chat/completions` with an `Authorization: Bearer` header and reads `choices[0].message.content`.
    *   **CORS and the local proxy**: Anthropic, OpenAI, and Gemini return the CORS headers a browser requires for a cross-origin call. Some gateways (notably **GenAI.mil**) do **not**, so the browser blocks the response regardless of network reachability. For those, the repo ships a small local proxy in two forms — `tools/ff-ai-proxy.ps1` (PowerShell/`HttpListener`, for systems without Python) and `tools/ff-ai-proxy.py` (Python stdlib) — that serves the dashboard *and* relays its API calls server-to-server (where CORS does not apply). Ticking **"Route through local proxy"** sends the request to a same-origin `/__ai_proxy` path (so no CORS) with an `X-FF-Target` header naming the real upstream; the proxy forwards the body and auth headers verbatim and returns the upstream response. The key still lives only in the browser and passes through the local proxy exactly as it would directly. Where even the proxy is impossible — **Constrained Language Mode** (WDAC/AppLocker) blocks the `HttpListener` it needs — a **Copy as PowerShell command** button instead emits a ready-to-run `Invoke-RestMethod` call (endpoint, key, and the selected data as the body). `Invoke-RestMethod` is a cmdlet (runs under CLM) and PowerShell is not a browser (no CORS), so the analyst pastes it into a console and reads the answer there; verified working under Constrained Language Mode.
...
5.  **Gateway key-lock handling**: GenAI.mil auto-locks API keys every 8 hours and answers with `401` and an `{ error: { unlock_url } }` body. Both the chat and model-list paths parse this, and instead of a raw error the UI renders a clickable **Unlock key** link (accepted only when it is an `http(s)` URL, to block `javascript:`-style payloads) plus a **Retry** button that re-sends the same question once the key is re-enabled.
